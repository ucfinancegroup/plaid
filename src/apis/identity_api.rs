...
/// struct for typed errors of method `retrieve_identity`
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum RetrieveIdentityError {
...
pub async fn retrieve_identity(configuration: &configuration::Configuration, retrieve_identity_request: crate::models::RetrieveIdentityRequest) -> Result<crate::models::RetrieveIdentityExample, Error<RetrieveIdentityError>> {
    ...
    let local_var_uri_str = format!("{}/identity/get", configuration.base_path);
    let mut local_var_req_builder = local_var_client.post(local_var_uri_str.as_str());

    if let Some(ref local_var_user_agent) = configuration.user_agent {
        local_var_req_builder = local_var_req_builder.header(reqwest::header::USER_AGENT, local_var_user_agent.clone());
    }
    local_var_req_builder = local_var_req_builder.json(&retrieve_identity_request);
    ...
    if local_var_status.is_success() {
        ...
        let local_var_entity: Option<RetrieveIdentityError> = serde_json::from_str(&local_var_content).ok();
