...
pub trait IdentityApi {
    fn retrieve_identity(&self, retrieve_identity_request: crate::models::RetrieveIdentityRequest) -> Box<dyn Future<Item = crate::models::RetrieveIdentityExample, Error = Error<serde_json::Value>>>;
}

impl<C: hyper::client::Connect>IdentityApi for IdentityApiClient<C> {
    fn retrieve_identity(&self, retrieve_identity_request: crate::models::RetrieveIdentityRequest) -> Box<dyn Future<Item = crate::models::RetrieveIdentityExample, Error = Error<serde_json::Value>>> {
        let mut req = __internal_request::Request::new(hyper::Method::Post, "/identity/get".to_string())
        ;
        req = req.with_body_param(retrieve_identity_request);
