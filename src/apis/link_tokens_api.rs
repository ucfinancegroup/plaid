...
/// struct for typed errors of method `create_link_token`
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum CreateLinkTokenError {
...
pub async fn create_link_token(configuration: &configuration::Configuration, create_link_token_request: crate::models::CreateLinkTokenRequest) -> Result<(), Error<CreateLinkTokenError>> {
    ...
    let local_var_uri_str = format!("{}/link/token/create", configuration.base_path);
    let mut local_var_req_builder = local_var_client.post(local_var_uri_str.as_str());

    if let Some(ref local_var_user_agent) = configuration.user_agent {
        local_var_req_builder = local_var_req_builder.header(reqwest::header::USER_AGENT, local_var_user_agent.clone());
    }
    local_var_req_builder = local_var_req_builder.json(&create_link_token_request);
