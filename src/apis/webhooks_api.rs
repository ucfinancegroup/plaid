...
/// struct for typed errors of method `fire_webhook_sandbox_only`
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum FireWebhookSandboxOnlyError {
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method `get_webhook_verification_key`
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetWebhookVerificationKeyError {
    UnknownValue(serde_json::Value),
}
...
pub async fn fire_webhook_sandbox_only(configuration: &configuration::Configuration, fire_webhook_sandbox_only_request: crate::models::FireWebhookSandboxOnlyRequest) -> Result<(), Error<FireWebhookSandboxOnlyError>> {
    ...
    let local_var_uri_str = format!("{}/sandbox/item/fire_webhook", configuration.base_path);
    let mut local_var_req_builder = local_var_client.post(local_var_uri_str.as_str());

    if let Some(ref local_var_user_agent) = configuration.user_agent {
        local_var_req_builder = local_var_req_builder.header(reqwest::header::USER_AGENT, local_var_user_agent.clone());
    }
    local_var_req_builder = local_var_req_builder.json(&fire_webhook_sandbox_only_request);
...
pub async fn get_webhook_verification_key(configuration: &configuration::Configuration, get_webhook_verification_key_request: crate::models::GetWebhookVerificationKeyRequest) -> Result<(), Error<GetWebhookVerificationKeyError>> {
    ...
    let local_var_uri_str = format!("{}/webhook_verification_key/get", configuration.base_path);
    let mut local_var_req_builder = local_var_client.post(local_var_uri_str.as_str());

    if let Some(ref local_var_user_agent) = configuration.user_agent {
        local_var_req_builder = local_var_req_builder.header(reqwest::header::USER_AGENT, local_var_user_agent.clone());
    }
    local_var_req_builder = local_var_req_builder.json(&get_webhook_verification_key_request);
